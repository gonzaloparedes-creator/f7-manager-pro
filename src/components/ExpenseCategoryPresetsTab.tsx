...
export default function ExpenseCategoryPresetsTab() {
  ...
  const add = async () => {
    ...
    setSaving(true);
    const { error } = await supabase.from("expense_categories").insert({ company_id: companyId, label: trimmed });
    setSaving(false);
    if (error) return toast({ title: "Error", description: error.message, variant: "destructive" });
    setLabel("");
    toast({ title: "Categoría de gasto agregada" });
    load();
  };

  const remove = async (p: Preset) => {
    setDeleting(true);
    const { error } = await supabase.from("expense_categories").delete().eq("id", p.id);
    setDeleting(false);
    if (error) return toast({ title: "Error", description: error.message, variant: "destructive" });
    ...
    <Card>
      <CardContent className="space-y-4 p-6">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-semibold">Categorías de gastos</div>
            <div className="text-xs text-muted-foreground">
              Opciones disponibles al cargar un gasto en el módulo de Gastos.
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
          <div className="space-y-2">
            <Label htmlFor="expcat_label">Nombre / Etiqueta</Label>
            <Input
              id="expcat_label"
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); add(); } }}
              placeholder="Marketing / Impuestos / Mantenimiento..."
            />
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-md border bg-muted/30 p-6 text-center text-sm text-muted-foreground">
            Aún no hay categorías de gastos configuradas.
          </div>
        ) : (
