...
export default function PaymentMethodPresetsTab() {
  ...
  const add = async () => {
    ...
    setSaving(true);
    const { error } = await supabase.from("payment_method_presets").insert({ company_id: companyId, label: trimmed });
    setSaving(false);
  ...
  const remove = async (p: Preset) => {
    setDeleting(true);
    const { error } = await supabase.from("payment_method_presets").delete().eq("id", p.id);
    setDeleting(false);
    if (error) return toast({ title: "Error", description: error.message, variant: "destructive" });
      ...
      <CardContent className="space-y-4 p-6">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Opciones disponibles al cobrar una seña o saldo (Nueva Orden, Modo Lote, Presupuestos).
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
          <div className="space-y-2">
            <Label htmlFor="pm_label">Nombre / Etiqueta</Label>
            <Input
              id="pm_label"
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); add(); } }}
              placeholder="Giro / QR / Cheque..."
            />
          </div>
