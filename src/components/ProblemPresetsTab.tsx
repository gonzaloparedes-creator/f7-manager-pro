...
export default function ProblemPresetsTab() {
  ...
  const add = async () => {
    ...
    setSaving(true);
    const { error } = await supabase.from("problem_presets").insert({ company_id: companyId, label: trimmed });
    setSaving(false);
    if (error) return toast({ title: "Error", description: error.message, variant: "destructive" });
    setLabel("");
    toast({ title: "Problema agregado" });
    load();
  };

  const remove = async (p: Preset) => {
    setDeleting(true);
    const { error } = await supabase.from("problem_presets").delete().eq("id", p.id);
    setDeleting(false);
    if (error) return toast({ title: "Error", description: error.message, variant: "destructive" });
      ...
      <CardContent className="space-y-4 p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
          <div className="space-y-2">
            <Label htmlFor="pp_label">Nombre / Etiqueta</Label>
            <Input
              id="pp_label"
              value={label}
        ...
        {loading ? (
          ...
          <div className="rounded-md border bg-muted/30 p-6 text-center text-sm text-muted-foreground">
            Aún no hay problemas configurados.
          </div>
        ) : (
