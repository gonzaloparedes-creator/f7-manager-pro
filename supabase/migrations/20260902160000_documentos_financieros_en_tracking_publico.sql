-- El cliente no podía ver los PDF adjuntos (factura, garantía del
-- fabricante, etc.) desde el link de seguimiento — financial_documents
-- no se exponía en get_order_by_code/get_order_by_tracking. Se agrega
-- como columna más; el frontend ya decide mostrar la sección solo si
-- hay documentos cargados.
--
-- Mismo cuerpo que la versión de
-- 20260902150000_marca_modelo_imei_en_tracking_publico.sql, sumando
-- o.financial_documents.
...
CREATE FUNCTION public.get_order_by_code(_code text) RETURNS TABLE(
  ...
  marca text, modelo text, imei text, financial_documents jsonb
)
  ...
  SELECT o.id, o.order_number, o.device_type, o.status, o.technician_notes,
         ...
         o.marca, o.modelo, o.imei, o.financial_documents
